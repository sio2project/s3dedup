use crate::AppState;
use crate::filetracker_client::FiletrackerClient;
...
pub use strategies::migrate_single_file_from_metadata;
pub use strategies::migrate_single_file_from_streaming;

use strategies::migrate_single_file;
use strategies::migrate_single_file_from_v1_fs;
...
/// Calls `attempt_fn` up to `max_retries + 1` times. Ok(true) = migrated, Ok(false) = skipped.
async fn retry_migration<F, Fut>(
    label: &str,
    path: &str,
    max_retries: u32,
    attempt_fn: F,
...
async fn migrate_single_file_with_retry(
    ...
    retry_migration("FT", path, max_retries, || {
        migrate_single_file(filetracker_client, app_state.clone(), path)
    })
    .await
}
...
async fn migrate_single_file_from_v1_fs_with_retry(
    ...
    retry_migration("V1", &file_info.relative_path, max_retries, || {
        migrate_single_file_from_v1_fs(app_state.clone(), file_info)
    })
    .await
}
